...
const _download = download._download;
    ...
    download._download = async function (options) {
        console.log('=======call donwload funct---------')
        if (session.bg_color) {
            console.log('--custom-session mobile obile----',odoo.csrf_token)
            if (odoo.csrf_token) {
            ...
            console.log('--custom-session mobile opt data----',option_data)
    ...
    patch(ActionMenus.prototype, "spiffy_theme_backend.actionmenuinherit", {
      async willUpdateProps(nextProps) {
        var is_tree_form_split = $('.split_view_separator').length
        if (is_tree_form_split){
