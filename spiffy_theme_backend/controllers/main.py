...
from odoo.addons.web.controllers.main import Home as WebHome, Session as WebSession,DataSet as primary_colorDataset,ensure_db,GroupsTreeNode,ExportXlsxWriter,GroupExportXlsxWriter
...
class BackendConfigration(http.Controller):
    ...
    @http.route('/text_color/label_color',type="json",auth="user")
        ...
        if 'options' in kw:
            if 'file_generator' and 'options' in kw['options']:
                ...
                if type_ in ['qweb-pdf', 'qweb-text']:
                    ...
                    report_data = report.sudo().with_context(context)._render_qweb_pdf(docids, data=data)[0]
